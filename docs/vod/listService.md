...
### 描述
```
1、请求入参startTime（开始时间）和endTime（结束时间）的优先级最高
2、当请求入参startTime和endTime都不为空，则startTime＜＝时间区间<=endTime
3、当请求入参startTime为空，endTime不为空，则时间区间为<=endTime
4、当请求入参startTime不为空，endTime为空，则时间区间为>=startTime
5、当请求入参startTime和endTime都为空，则时间区间以startDate（开始日期）和endDate（结束日期）为准
```
### 调用约束
1、接口调用有频率限制，[详细请查看](/limit.md)，调用常见异常，[详细请查看](/exceptionDoc)
