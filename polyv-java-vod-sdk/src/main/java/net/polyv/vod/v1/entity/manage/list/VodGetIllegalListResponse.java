...
@ApiModel("获取不通过视频列表返回实体")
public class VodGetIllegalListResponse extends VodPageCommonResponse {
    
    /**
     * 返回的结果集
     */
    @ApiModelProperty(name = "contents", value = "返回的结果集", required = false)
    private List<IllegalVideoInfo> contents;
    
    @Data
    @Accessors(chain = true)
    @ApiModel("不通过视频信息")
    public static class IllegalVideoInfo {
        /**
         * 视频标签
         */
